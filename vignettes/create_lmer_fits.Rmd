...
```r
set.seed(1204)
bootsamplesdata<-boots.samples(dat=example.dat,sub.id ="subjects",B=100)#change back to 5000 later
usethis::use_data(bootsamplesdata)
```

But we will loaded from the package

```{r}
data(bootsamplesdata, package="boots.lmer")
```
